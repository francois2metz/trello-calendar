...
app.get('/', function(req, res) {
    if (req.trello) {
        stats.agg('root_logged', '1c');
        res.render('index_logged', {uuid: req.session.uuid, config: config});
    } else {
        stats.agg('root_unlogged', '1c');
        res.render('index_unlogged',{https: req.protocol == 'https', config: config});
    }
});
